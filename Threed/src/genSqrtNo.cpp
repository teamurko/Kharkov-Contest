...
vp boundingBox(vp v) {
	...
	double DY=maxY-minY;
	double X,Y;
	if (myRand()%2) X=minX+0.01*DX*0; else X=maxX-0.01*DX*0;
	if (myRand()%2) Y=minY+0.01*DY*0; else Y=maxY-0.01*DX*0;
	X=int(X);
	Y=int(Y);
	v.pb(P(X,Y));
	v=convexHull(v);
...
bool checkBoundingBox(vp &xy, vp &yz, vp &zx) {
...
void genBoundingBox(vp3 v) {
	...
	int shift=1;
	int r=myRand()%3;
	if (r==0) xy=boundingBox(xy);
	if (r==1) yz=boundingBox(yz);
	if (r==2) zx=boundingBox(zx);
	outVP(xy);
	outVP(yz);
	outVP(zx);
}

void genTestYes(vp3 v) {
	vp xy,yz,zx;
	forn(i,sz(v)) {
		xy.pb(P(v[i].x,v[i].y));
		yz.pb(P(v[i].y,v[i].z));
	...
	for(int step=0; step<5; step++) {
		...
		P &p=v[myRand()%sz(v)];
		int dx=myRand()%(2*shift+1)-shift;
		int dy=myRand()%(2*shift+1)-shift;
		vp tmp=v;
		p.x+=dx;
		p.y+=dy;
		v=convexHull(v);
		if (!checkAll(xy,yz,zx)) {
			v.assign(all(tmp));
			break;
		}
	}
...
int main(int argc, char **argv) {
	...
	genBoundingBox(genLargeVP3(n,maxX));
	return 0;
}